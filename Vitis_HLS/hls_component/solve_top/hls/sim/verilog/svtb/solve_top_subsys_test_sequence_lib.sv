...
//
//==============================================================
`ifndef SOLVE_TOP_SUBSYS_TEST_SEQUENCE_LIB__SV                                              
    `define SOLVE_TOP_SUBSYS_TEST_SEQUENCE_LIB__SV                                          
                                                                                                    
                                                                                                    
    `include "uvm_macros.svh"                                                                     
                                                                                                    
    class solve_top_subsys_test_sequence_lib extends uvm_sequence;                                
                                                                                                    
        function new (string name = "solve_top_subsys_test_sequence_lib");                      
            super.new(name);                                                                        
            `uvm_info(this.get_full_name(), "new is called", UVM_LOW)                             
        endfunction                                                                                 
                                                                                                    
        `uvm_object_utils(solve_top_subsys_test_sequence_lib)                                     
        `uvm_declare_p_sequencer(solve_top_virtual_sequencer)                                     
        ...
        virtual task body();                                                                        
            ...
            axi_pkg::axi_busdatas_master_sequence#(7, 32) axi_master_wr_CTRL_seq;
            axi_pkg::axi_busdatas_master_sequence#(7, 32) axi_master_rd_CTRL_seq;
            axi_pkg::axi_busdatas_master_sequence#(7, 32) axi_master_poll_CTRL_seq;

            if (!uvm_config_db#(solve_top_reference_model)::get(p_sequencer,"", "refm", refm))
                    ...
                    fork
                        begin
                            ...
                            for(int i=0; i<3; i++) begin
                                ...
                                logic[63:0] data64bit_b_in[$];
                                logic[32-1:0] databusbit_b_in[$];
                                data64bit_A_in.delete(); databusbit_A_in.delete();
                                axi_master_wr_CTRL_seq.StableAxiliteNoUpdate=0;
                                refm.mem_blk_pages_CTRL_A_in.tobusdata(data64bit_A_in, refm.mem_blk_pages_CTRL_A_in.rd_page_idx, 32);
                                foreach(data64bit_A_in[s]) databusbit_A_in[s]=data64bit_A_in[s][32-1:0];
                                axi_master_wr_CTRL_seq.StableAxiliteNoUpdate=1;
                                axi_master_wr_CTRL_seq.datamerge_inavg(databusbit_A_in, 0, 64, 1);
                                data64bit_b_in.delete(); databusbit_b_in.delete();
                                axi_master_wr_CTRL_seq.StableAxiliteNoUpdate=0;
                                refm.mem_blk_pages_CTRL_b_in.tobusdata(data64bit_b_in, refm.mem_blk_pages_CTRL_b_in.rd_page_idx, 32);
                                foreach(data64bit_b_in[s]) databusbit_b_in[s]=data64bit_b_in[s][32-1:0];
                                axi_master_wr_CTRL_seq.StableAxiliteNoUpdate=1;
                                axi_master_wr_CTRL_seq.datamerge_inavg(databusbit_b_in, 0, 16, 1);
                                ...
                                `uvm_info("CTRL data writting thread", $sformatf("%0dth(total 3): waiting for all write data finish event",i), UVM_LOW)
                                ...
                                fork
                                    begin
                                        ...
                                        `uvm_info("CTRL start dut by axilite", $sformatf("%0dth(total 3): begin to set start bit",i), UVM_LOW)
                                        `uvm_send(axi_master_wr_CTRL_seq);
                                    end
                                    begin
                                        `uvm_info("CTRL wait for ap_ready for next trans", $sformatf("%0dth(total 3): begin to wait",i), UVM_LOW)
                        ...
                        begin
                            ...
                            for(int j=0; j<3; j=j+refm.ap_done_cnt) begin
                                ...
                                @refm.dut2tb_ap_done;
                                axi_master_rd_CTRL_seq.datamerge_inavg(databusbit, (4*32+32-1)/32, 32, 0);
                                axi_master_rd_CTRL_seq.datamerge_inavg(databusbit, (1*16+32-1)/32, 48, 0);
