# 1 "C:/ELEC522/sm512/asdf/fpga_hls/qrd.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 396 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "C:/ELEC522/sm512/asdf/fpga_hls/qrd.cpp" 2

# 1 "C:/ELEC522/sm512/asdf/fpga_hls/linearsolver.h" 1
...
# 4 "C:/ELEC522/sm512/asdf/fpga_hls/linearsolver.h" 2
# 1 "C:/Xilinx/2025.1/Vitis/common/technology/autopilot\\ap_int.h" 1
# 5 "C:/ELEC522/sm512/asdf/fpga_hls/linearsolver.h" 2
...
# 31 "C:/ELEC522/sm512/asdf/fpga_hls/linearsolver.h"
    ...
    static const FIXED_TYPE atan_table[13] = {
    ...
    static inline bool near_zero(FIXED_TYPE v, int frac_bits = (16 - 5)) {
    ...
    static inline void cordic_atan(FIXED_TYPE x_in, FIXED_TYPE y_in,
        ...
        VITIS_LOOP_81_1: for (int i = 0; i < 13; i++) {
#pragma HLS pipeline off
 FIXED_TYPE d = (y >= ZERO) ? FIXED_TYPE(-1) : FIXED_TYPE(1);
    ...
    static inline void cordic_rotate(FIXED_TYPE xi, FIXED_TYPE yi, FIXED_TYPE theta,
        ...
        VITIS_LOOP_107_1: for (int i = 0; i < 13; i++) {
    ...
    void decompose_qr(FIXED_TYPE matrix_in[4][8],
        FIXED_TYPE QT_out[4][4],
        FIXED_TYPE R_out[4][4],
        short* done);
    ...
    void upper_backsub(FIXED_TYPE R_in[4][4],
        FIXED_TYPE y_in[4],
        FIXED_TYPE x_out[4],
        short* done);


    void solve_top(float A_in[4][4],
        float b_in[4],
        float x_out[4],
        short* done);
...
# 3 "C:/ELEC522/sm512/asdf/fpga_hls/qrd.cpp" 2

extern "C" void decompose_qr(FIXED_TYPE matrix_in[4][8],
                             FIXED_TYPE QT_out[4][4],
                             FIXED_TYPE R_out[4][4],
                             short *done) {
...
#pragma HLS INTERFACE s_axilite port=done bundle=CTRL
 ...
 FIXED_TYPE local_aug[4][8];
#pragma HLS ARRAY_PARTITION variable=local_aug complete dim=2


 VITIS_LOOP_18_1: for (int r = 0; r < 4; r++) {
        VITIS_LOOP_19_2: for (int c = 0; c < 8; c++) {
            local_aug[r][c] = matrix_in[r][c];
        }
    }


    VITIS_LOOP_25_3: for (int k = 0; k < 4; k++) {
        VITIS_LOOP_26_4: for (int j = k + 1; j < 4; j++) {
            ...
            VITIS_LOOP_39_5: for (int col = k; col < 4; col++) {
#pragma HLS PIPELINE II=1
 FIXED_TYPE xi = local_aug[k][col];
            ...
            VITIS_LOOP_50_6: for (int col = 4; col < 8; col++) {
#pragma HLS PIPELINE II=1
 FIXED_TYPE xi = local_aug[k][col];
                FIXED_TYPE xj = local_aug[j][col];
                FIXED_TYPE new_i, new_j;
        ...
        if (local_aug[k][k] < ZERO) {
            VITIS_LOOP_63_7: for (int c = 0; c < 8; c++) {
    ...
    VITIS_LOOP_70_8: for (int r = 0; r < 4; r++) {
        VITIS_LOOP_71_9: for (int c = 0; c < 4; c++) {
            R_out[r][c] = (c >= r) ? local_aug[r][c] : FIXED_TYPE(0);
        }
    }


    VITIS_LOOP_77_10: for (int r = 0; r < 4; r++) {
        VITIS_LOOP_78_11: for (int c = 0; c < 4; c++) {
            QT_out[r][c] = local_aug[r][c + 4];
        }
    }
