# 1 "C:/ELEC522/sm512/asdf/fpga_hls/matrixmult.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 396 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "C:/ELEC522/sm512/asdf/fpga_hls/matrixmult.cpp" 2
...
# 3 "C:/ELEC522/sm512/asdf/fpga_hls/matrixmult.cpp" 2
...
#pragma HLS INTERFACE s_axilite port=return bundle=CTRL
 ...
 FIXED_TYPE mat_local[4][4];
    FIXED_TYPE vec_local[4];
    FIXED_TYPE res_local[4];
...
#pragma HLS ARRAY_PARTITION variable=res_local complete dim=1
 ...
 VITIS_LOOP_24_1: for (int i = 0; i < 4; i++) {
        VITIS_LOOP_25_2: for (int j = 0; j < 4; j++) {
            mat_local[i][j] = mat[i][j];
        }
    }
    VITIS_LOOP_29_3: for (int k = 0; k < 4; k++) {
        vec_local[k] = vec[k];
    }


    VITIS_LOOP_34_4: for (int i = 0; i < 4; i++) {
#pragma HLS PIPELINE II=1
 res_local[i] = FIXED_TYPE(0);
    }


    VITIS_LOOP_40_5: for (int i = 0; i < 4; i++) {
#pragma HLS PIPELINE II=1
 FIXED_TYPE sum = FIXED_TYPE(0);
        VITIS_LOOP_43_6: for (int k = 0; k < 4; k++) {
    ...
    VITIS_LOOP_50_7: for (int i = 0; i < 4; i++) {
#pragma HLS PIPELINE II=1
 res[i] = res_local[i];
    }
}
