# 1 "C:/ELEC522/sm512/asdf/fpga_hls/linearsolver.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 396 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "C:/ELEC522/sm512/asdf/fpga_hls/linearsolver.cpp" 2
...
#pragma HLS pipeline off
 FIXED_TYPE d = (y >= ZERO) ? FIXED_TYPE(-1) : FIXED_TYPE(1);
    ...
    __attribute__((sdx_kernel("solve_top", 0))) void solve_top(float A_in[4][4],
        float b_in[4],
        float x_out[4],
        short* done);
...
# 7 "C:/ELEC522/sm512/asdf/fpga_hls/linearsolver.cpp" 2
...
{
#line 1 "directive"
#pragma HLSDIRECTIVE TOP name=solve_top
# 16 "C:/ELEC522/sm512/asdf/fpga_hls/linearsolver.cpp"
...
#pragma HLS INTERFACE s_axilite port=done bundle=CTRL
 ...
 FIXED_TYPE aug_mat[4][8];
    FIXED_TYPE Qt_mat[4][4];
    FIXED_TYPE R_mat[4][4];
    FIXED_TYPE b_fix[4];
    FIXED_TYPE y_fix[4];
    FIXED_TYPE x_fix[4];
...
#pragma HLS ARRAY_PARTITION variable=x_fix complete dim=1
 ...
 VITIS_LOOP_39_1: for (int r = 0; r < 4; ++r) {
#pragma HLS UNROLL
 VITIS_LOOP_41_2: for (int c = 0; c < 8; ++c) {
#pragma HLS UNROLL
 if (c < 4) {
                aug_mat[r][c] = (FIXED_TYPE) A_in[r][c];
            } else {
                aug_mat[r][c] = (c - 4 == r) ? FIXED_TYPE(1) : FIXED_TYPE(0);
            }
        }
    }


    VITIS_LOOP_52_3: for (int i = 0; i < 4; ++i) {
#pragma HLS UNROLL
 b_fix[i] = (FIXED_TYPE) b_in[i];
    ...
    VITIS_LOOP_69_4: for (int i = 0; i < 4; ++i) {
#pragma HLS UNROLL
